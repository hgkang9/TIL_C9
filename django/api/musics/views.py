...
@api_view(['GET']) #어떠한 method만 허용할 건지
def music_list(request):
    musics=Music.objects.all()
    serializer=MusicSerializer(musics, many=True) #music이 여러개이므로 넣어주는 옵션
    #Serializer
...
def music_detail(request, music_id):
    ...
    serializer=MusicSerializer(music) #detail 페이지에는 music이 하나만 있으므로 many=True 필요없다
    return Response(serializer.data)
...
def comment_create(request, music_id):
    ...
    if serializer.is_valid(raise_exception=True): #잘못된 요청일 시 error 메시지를 보여준다(응답을 해줌)
